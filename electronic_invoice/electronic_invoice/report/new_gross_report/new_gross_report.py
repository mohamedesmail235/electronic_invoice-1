...
class GrossProfitGenerator(object):
	...
	def load_invoice_items(self):
		...
		##ADDED##
		if self.filters.customer_group_list and self.filters.group_by == 'Customer Group':
			conditions += " and `tabSales Invoice`.customer_group in %(customer_group_list)s"
			#conditions += " and `tabSales Invoice`.customer_group in (%s)" % (', '.join(['%s']*len(self.filters.customer_group_list)))#, tuple(self.filters.customer_group_list))# (','.join(['%s'] * len(self.filters.customer_group_list)))
		###END ADDED##
		...
		self.si_list = frappe.db.sql("""
			...
			.format(conditions=conditions, sales_person_cols=sales_person_cols,
				sales_team_table=sales_team_table, match_cond = get_match_cond('Sales Invoice')), self.filters, as_dict=1)#tuple([self.filters] + self.filters.customer_group_list)
